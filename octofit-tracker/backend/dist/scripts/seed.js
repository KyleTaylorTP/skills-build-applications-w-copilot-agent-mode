...
async function seedDatabase() {
    try {
        ...
        const createdTeams = await Team_1.default.insertMany(teamsToInsert);
        ...
        const createdActivities = await Activity_1.default.insertMany(activitiesToInsert);
        console.log(`✓ Created ${createdActivities.length} activities`);
        // Seed workouts with actual user IDs
        const workoutsToInsert = createdUsers.flatMap((user, userIndex) => sampleWorkouts.slice(0, userIndex + 1).map((workout) => ({
            ...workout,
            userId: user._id,
        })));
        const createdWorkouts = await Workout_1.default.insertMany(workoutsToInsert);
        ...
        const createdLeaderboard = await Leaderboard_1.default.insertMany(leaderboardData);
